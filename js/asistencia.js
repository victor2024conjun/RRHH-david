...
async function recalcDia(colabId, fecha) {
  const c = colabById(colabId), t = turnoById(c?.turno_id) || DEFAULT_TURNO, tol = t.tolerancia_min ?? 5;
  ...
  (data || []).forEach((m, i) => {
    ...
    if (pu.puntualidad !== m.puntualidad || pu.diff_min !== m.diff_min) ups.push(sb.from('marcaciones').update(pu).eq('id', m.id));
  });
  await Promise.all(ups);
}

VIEWS.asistencia = async el => {
  const tab = S.asisTab || 'asis';
  el.innerHTML = pageHead('Asistencia', 'Marcaciones, horas trabajadas y turnos') +
    `<div class="tabs"><div class="tab ${tab === 'asis' ? 'active' : ''}" data-t="asis">Asistencias</div><div class="tab ${tab === 'turnos' ? 'active' : ''}" data-t="turnos">Turnos configurados</div></div><div id="aBody"></div>`;
  $$('.tab', el).forEach(t => t.onclick = () => { S.asisTab = t.dataset.t; go('asistencia'); });
  await (tab === 'asis' ? tabAsistencias : tabTurnos)($('#aBody'));
};
...
async function tabAsistencias(el) {
  ...
  const load = async () => {
    ...
    const rows = S.colabs.filter(c => c.estado === 'activo' && (!ar || c.area === ar) && (!q || `${nombreCompleto(c)} ${c.cedula}`.toLowerCase().includes(q))).map(c => {
      const ms = by[c.id] || [], t = turnoById(c.turno_id), r = calcHoras(ms, () => t, lun, fecha, new Date());
...
async function detalleColab(c, desde, hasta, onChange) {
  ...
  const pintar = async () => {
    ...
    const res = calcHoras(marks, () => t, d1, d2, new Date()), rows = filasDias(res, marks.filter(x => x.fecha >= d1 && x.fecha <= d2));
    m.body.innerHTML = `<div class="row mb"><div><label>Desde</label><input type="date" id="dD" value="${d1}"></div><div><label>Hasta</label><input type="date" id="dH" value="${d2}"></div>
      ...
      <div class="card" style="background:#f7f8fd;margin-bottom:12px"><b>${hm(res.totalMin)}</b> laboradas · <b>${hm(res.extraMin)}</b> extras · <b>${res.diasMarcados}</b> días marcados · Turno: ${h(t?.nombre || 'Sin turno')} (${turnoTxt(t)})</div>
      ${rows.map(r => `<div class="card" style="margin-bottom:10px"><div class="row between"><b>${r.dow} ${fmtDate(r.fecha)}</b><span>${hm(r.total)} ${r.extra ? `· <span class="badge amber">${hm(r.extra)} extra</span>` : ''} ${r.sinSalida ? '<span class="badge red">Sin salida</span>' : ''}</span></div>
  ...
  const m = modal({
    ...
    buttons: [{ t: 'Cancelar' }, {
      t: 'Guardar', cls: 'primary', fn: async () => {
        ...
        const row = { colaborador_id: c.id, fecha: v.fecha, ts: ts.toISOString(), tipo: v.tipo, nota: v.nota || (mk ? 'Modificada por administrador' : 'Registro manual'), origen: 'manual', turno_id: c.turno_id };
...
async function tabTurnos(el) {
  await loadBase();
  el.innerHTML = `<div class="row between mb"><p class="muted" style="margin:0">Cree jornadas continuas o partidas. La tolerancia por defecto es de ${CFG.params.tolerancia_min} minutos.</p><button class="btn primary" id="tNew">+ Nuevo turno</button></div>
  ...
  $$('[data-x]', el).forEach(b => b.onclick = async () => {
    ...
    const { error } = await sb.from('turnos').delete().eq('id', b.dataset.x); if (error) return toast(dbErr(error), 'err'); go('asistencia');
  });
}
  ...
  const m = modal({
    ...
    buttons: [{ t: 'Cancelar' }, {
      t: 'Guardar', cls: 'primary', fn: async () => {
        ...
        if (error) { toast(dbErr(error), 'err'); return false; } toast('Turno guardado', 'ok'); go('asistencia');
      }
    }]
  });
  const upd = () => {
...
async function computeReport(list, desde, hasta) {
  ...
  return list.map(c => {
    const ms = by[c.id] || [], t = turnoById(c.turno_id), res = calcHoras(ms, () => t, desde, hasta, new Date());
