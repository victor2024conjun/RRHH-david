...
VW.inicio = async el => {
  ...
  const marks = asis ? await fetchMarcas([me.id], lun, hoy) : [], t = P.turno, r = calcHoras(marks, () => t, lun, hoy, new Date());
...
VW.registros = async el => {
  ...
  const run = async () => {
    ...
    const marks = await fetchMarcas([me.id], ymd(mondayOf(parseYmd(d))), hh), res = calcHoras(marks, () => t, d, hh, new Date()), inR = marks.filter(m => m.fecha >= d && m.fecha <= hh), rows = filasDias(res, inR); cur = { d, hh, res, inR };
    const T = res.total;
...
/* ---------------- PERMISOS ---------------- */
const TIPOS_PERM = [['permiso_remunerado', 'Permiso remunerado'], ['permiso_no_remunerado', 'Permiso no remunerado'], ['incapacidad', 'Incapacidad médica'], ['licencia', 'Licencia (luto, maternidad, paternidad…)'], ['vacaciones', 'Vacaciones'], ['otro', 'Otro documento']];
VW.permisos = async el => {
  ...
  el.innerHTML = ph('Permisos e incapacidades', 'Solicite un permiso y adjunte el soporte si es necesario') +
    ...
    <div class="field"><label>Desde</label><input type="date" id="pD" value="${ymd(new Date())}" required></div><div class="field"><label>Hasta</label><input type="date" id="pH" value="${ymd(new Date())}" required></div>
  ...
  $('#pF').onsubmit = async ev => {
    ...
    try {
      ...
      const { error } = await sb.from('permisos').insert({ colaborador_id: P.me.id, tipo: $('#pT').value, fecha_inicio: d, fecha_fin: hh, motivo: $('#pM').value.trim(), documento_url: url, documento_nombre: f?.name || null });
      if (error) throw error;
      notify('permiso', `📝 Solicitud de permiso: ${nombreCompleto(P.me)}`, `${$('#pT').selectedOptions[0].text} · ${fmtDate(d)} a ${fmtDate(hh)}`, P.me.id);
...
VW.perfil = async el => {
  ...
  $('#sP').onclick = async () => {
    const v = { foto_url: $('#fFoto').value.trim() || null, correo: $('#fCor').value.trim() || null, telefono: $('#fTel').value.trim() || null, direccion: $('#fDir').value.trim() || null, banco: $('#fBan').value.trim() || null, cuenta_bancaria: $('#fCta').value.trim() || null };
